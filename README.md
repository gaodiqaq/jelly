...
- **「乌梅冻」深色主题**：深乌梅底 + 蜜桃冻强调色，半透明"果冻方块"作为品牌标识与 AI 头像，Agent 思考时会轻轻颤动
...
- **流式输出**：模型回复实时渲染，token 用量与缓存命中率实时展示，可随时停止
...
│   └── builtins/   # 内置 Skill
├── core/           # 会话持久化、工具执行器（权限/审批）、Agent 状态机
├── server/         # FastAPI：REST + WebSocket、会话管理、多用户隔离、权限审批、工作台 API
├── ui/             # rich 渲染、REPL 输入、权限询问
└── main.py         # typer CLI 入口
webui/              # React + Vite 前端（dist/ 由服务端托管）
Dockerfile          # 多阶段构建
